...
// //写log和打印的方法
// void printfAndWriteMyLogFun(const char *format,...){
//   va_list arg;
//
//   va_start (arg, format);
//
//   printf("%s", arg);
//
//   writeMyLog(file_OpenMyLog, "%s", arg);
//
//   va_end (arg);
//
//
...
int ping()
{
  ...
  while(k==0)
  {
    ...
    {
      printfAndWriteMyLogFun("网络连接失败，无法连接 MQTT 服务器！");
      return 0;
    }
...
int linkOneNET() {
  ...
  int main(int argc, char* argv[])
  {
    ...
    printfAndWriteMyLogFun("RaspberryPiTemp: %.2f\n", getRaspiTemp());
