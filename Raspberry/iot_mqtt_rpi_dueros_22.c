...
int messageArrived(void *context, char *topicName, int topicLen, MQTTClient_message *message)
{
  ...
  char* jsonNameKey;
  char* jsonNameVolue;
  char* jsonDeviceIdKey;
  char* jsonDeviceIdVolue;
  cJSON * root = NULL;
  cJSON * name = NULL;//cjson对象
  cJSON * deviceId = NULL;//cjson对象
  ...
  else
  {
    ...
    jsonNameKey = name->string;
    jsonNameVolue = name->valuestring;
    jsonDeviceIdKey = deviceId->string;
    jsonDeviceIdVolue = deviceId->valuestring;
    jsonNameVolue=jsonKeyToChinese(jsonNameVolue);
    jsonDeviceIdVolue=jsonVolueToChinese(jsonDeviceIdVolue);
  }
  printfAndWriteMyLogFun("收到MQTT消息！主题:%s,消息:%s,中文:%s%s\n", topicName,payloadptr,jsonNameVolue,jsonDeviceIdVolue);
