...
char dest_Json_msg[200];
//得出最终发送到服务端的控制设备的指令
void setInstructions(char *res){
  memset(dest_Json_msg,0,sizeof(dest_Json_msg)); //清空数组
  int i;
  ...
  for ( i = 0; i < domesticAppliance_leng; i++) {
    ...
    if(""!=instructions&&""!=domesticAppliances){
      strcat(dest_Json_msg,instructions);//加入数组
      strcat(dest_Json_msg,domesticAppliances);
    }
}
  ...
  while (NULL!=fgets(buff, sizeof(buff), file_OpenDuerOSLog))//这里fgets()是阻塞的
  {
    ...
    else
    {
      ...
      if(strpoint!=-1)
      {
        ...
        if(NULL != dest_Json_msg && instructions!="" && domesticAppliances!=""){
          char dest2[300];
          strcat(dest2,"{\"cmd\":\"");
          strcat(dest2,dest_Json_msg);
          strcat(dest2,"\"}");
...
int messageArrived(void *context, char *topicName, int topicLen, MQTTClient_message *message)
{
  ...
  printf("key: %s volue: %s\n", key,volue);
  printf("%s%s\n", jsonKeyToChinese(key),jsonVolueToChinese(volue));
